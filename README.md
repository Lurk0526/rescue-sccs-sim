...
|  +---------+  +----------+  +--------+ |
|  |  AIE    |->|EKF Tracker|->| LiDAR  | |
|  | Enhance |  |  Verify   |  |Validate| |
|  +---------+  +----------+  +--------+ |
+----------------------------------------+
|          Decision Layer (决策层)         |
|  +----------------+  +--------------+  |
|  |  ADC Clustering|  | TCFM State   |  |
|  |  eps=f(density)|  | Y/G/R Markers|  |
|  +----------------+  +--------------+  |
+----------------------------------------+
|         Execution Layer (执行层)         |
|  +---------+  +---------+  +---------+ |
|  | A* Path |  |Collision|  |Physical | |
|  | Finding |  |Avoidance|  | Marking | |
|  +---------+  +---------+  +---------+ |
+----------------------------------------+
             [ Robot <--> Environment ]
+----------------------------------------+
|         RescueEnvironment              |
|    80x80 Grid | 30% Obstacles | 50 Survivors |
+----------------------------------------+
```
